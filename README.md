...
## 프로젝트 소개
### "MediSeed"는 폐의약품 수거함 위치 정보 제공을 통해, 올바른 수거 문화를 지향하는 앱입니다.
...
- Data -> Domaim -> Presentation의 순서로 의존성이 주입되며, 고수준 계층이 됨에 따라 상위 컴포넌트는 하위 컴포넌트를 전혀 알지 못합니다.
...
### 🏠 [홈 화면]
- ${\textsf{\color{green}네이버 지도 API}}$를 기반으로, 폐의약품 수거함 위치를 마커로 표시합니다.
...
### 🗃️ [보관함 화면]
- 홈 화면에서 마커를 클릭하면 수거함 정보를 표시하는 바텀시트가 나타나고, 이때 좋아요 버튼을 누를 경우 보관함에서 확인 가능합니다.
- 보관된 수거함을 클릭할 경우, 해당 수거함 위치가 표시된 지도 화면으로 전환됩니다.
- 새싹이 화면에서 ${\textsf{\color{green}"약 버리기"}}$ 또는 ${\textsf{\color{green}"공유 하기"}}$ 버튼을 누를 경우, 보관함에서 총 횟수 눌렀는지 확인 가능합니다.
...
### 🌳 [새싹이 화면]
- ${\textsf{\color{green}"약 버리기"}}$, ${\textsf{\color{green}"공유 하기"}}$ 버튼으로 새싹이 캐릭터 키웁니다. (각 레벨마다 필요한 경험치가 점차 증가합니다.)
- 레벨 높아짐에 따라 새싹이 캐릭터의 외향이 변합니다.
- 1 ~ 5레벨까지 있으며, 5레벨이 넘어가면 나무가 1그루 추가됩니다.
- 약 버리기 버튼은 폐의약품을 수거함에 버리고 체크하는 기능으로 수거함 인근에서 일일 1회만 클릭 가능합니다.
- ${\textsf{\color{green}"약 버리기"}}$를 하면 바텀시트에서 해당 수거함의 총 수거 횟수 증가합니다.
- ${\textsf{\color{green}"공유 하기"}}$는 앱을 공유하는 기능으로 일일 3회만 경험치가 증가합니다.
